...
// Pulse Edit
- (NSString *)pathToPermanentlyStoreCachedResponseDataForUrl:(NSURL *)url {
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
		return nil;
	...
	path =  [path stringByAppendingPathComponent:[[[self class] keyForURL:url] stringByAppendingPathExtension:extension]];
	[[self accessLock] unlock];
	return path;    
}
